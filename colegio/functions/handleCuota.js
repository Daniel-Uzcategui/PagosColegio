    ...
    async (event) => {
      ...
      if (!event.data.before.exists) {
        ...
        for (const studentDoc of studentsSnapshot.docs) {
          let cuotaRef = admin.firestore().collection(`students/${studentDoc.id}/cuota_payments`).doc();
...
export const handlePayment = onDocumentWritten(
  "students/{studentId}/cuota_payments/{cuotaId}",
    async (event) => {
      ...
      if (cuotaDataAfter.RemainingAmountDue === cuotaDataBefore?.RemainingAmountDue) {
        return
      }
