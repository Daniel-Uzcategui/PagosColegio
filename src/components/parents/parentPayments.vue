<template>
    <q-dialog :model-value="showDialog" @hide="$emit('update:showDialog', false)">
      ...
      <AddPayment v-if="addPaymentDialog" @submitted="submitted = !submitted" :selectedCuotas="selected" :students="students" :show-dialog="addPaymentDialog" @update:show-dialog="updateAddPaymentDiag" :parent="parentRef" />
    </q-dialog>
  </template>
  ...
  const parentRef = ref(props.parent);
  ...
  const cuotas = ref([]);
  const students = useCollection(query(collection(db, 'students'), where('Padres', 'array-contains', parentRef.value.id)), { ssrKey: 'students-ssr' })
