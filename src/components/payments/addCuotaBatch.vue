    ...
    <q-card class="my-card">
        <q-card-section>
            <q-form @submit="onSubmit">
              ...
              <q-select multiple v-model="Grado" :options="grados" map-options emit-value label="Grado" />
              <HelpType v-model:model-value="selectedHelpType"></HelpType>
...
const emits = defineEmits(['show-dialog'])
        ...
        const Grado = ref([]); // Change Grado to an array for multiple selections
        ...
        const onSubmit = async () => {
            try {
                ...
                if (Grado.value.length) {
                    const response = await cuotaStore.addBatchCuota({ Alias: Alias.value, Monto: Monto.value, Periodo: Periodo.value, Grado: Grado.value, help: selectedHelpType.value });
                    ...
                    Notify.create({message: 'Agregue mínimo un valor a Grado'})
                }
            }
            catch (error) {
                console.error(error);
